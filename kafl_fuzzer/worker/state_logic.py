...
class FuzzingStateLogic:
    ...
    def handle_redqueen(self, payload, metadata):
        ...
        if not success and self.config.funky:
            success = self.__perform_redqueen(payload, metadata)
            if success:
                self.logger.warn("%s [funky] Redqueen success in 2nd round (id=%d)" % (self, metadata["id"]))
        if not success and self.config.funky:
            success = self.__perform_redqueen(payload, metadata)
            if success:
                self.logger.warn("%s [funky] Redqueen success in 3nd round (id=%d)" % (self, metadata["id"]))
    ...
    def __perform_redqueen(self, payload, metadata):
        ...
        if not orig_hash or not appended_hash:
            ...
            self.logger.info("%s Redqueen: Input %d not stable, skipping.." % (self, metadata["id"]))
            return False
        ...
        if colored_alternatives:
            ...
            self.logger.warn("%s Redqueen: Input %d not stable after coloring, skipping.." % (self, metadata["id"]))
            return False
